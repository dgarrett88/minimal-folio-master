...
const Welcome = () => {
    ...
    const handleContinue = () => {
      if (page < 3) {
        ...
        setFade(true);
        setNavigateToProjects(true); // Indicate that navigation should happen
      }
    };
    ...
    useEffect(() => {
      if (navigateToProjects) {
        const timer = setTimeout(() => {
          navigate('/projects');
        }, 500);  // Navigate after fade-out animation
        return () => clearTimeout(timer);
      }
    }, [navigateToProjects, navigate]);

    return (
      <FadeAnimation trigger={fade} duration={500} onFadeComplete={() => {
        if (page < 3) {
          setPage(page + 1);
        } else {
          navigate('/projects');
